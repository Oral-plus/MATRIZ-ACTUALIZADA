...
<html>
    ...
    <button id="showImageButton">
        <img src="./img/imagen.png" alt="Logo"> <!-- Reemplaza 'logo.png' con el nombre de tu logo -->
    </button>
    <div id="imageContainer">
        <img id="imageDisplay" src="./img/IMA.png" alt="Imagen">
    </div>
    ...
    <br>
      ...
      <br>
            ...
            <option value="JOSHOATH PEREZ">JOSHOATH PEREZ</option>
            <option value="MARIANA">MARIANA</option>
        
            <option value="ANA MILENA">ANA MILENA</option>
            
            <option value="JESICA ACEVEDO">JESICA ACEVEDO</option>
			</select>
			...
			<select class="seleccion" name="cosa" onchange="cambia()" required>
				...
				<option value="Dispensacion">Dispensación
				<option value="Fabricacion">Fabricación
                ...
                <option value="Verificacion_orden_limpieza_y_desinfeccion">Verificacion orden, limpieza y desinfeccion
                <option value="Verificacion_de_registros">Verificacion de registros
                <option value="Reunion">Reunion
                <option value="Capacitacion">Capacitacion
                <option value="Recibir_turno_calidad">Recibir turno calidad
                <option value="Entregar_turno_calidad">Entregar turno calidad
                <option value="Toma_de_Muestras_de_Materias_Primas">Toma de Muestras de Materias Primas
                <option value="Ingreso_y_verificacion_de_Material_de_empaque_y_Materia_Prima">Ingreso y verificacion de Material de empaque y Materia Prima
                <option value="Verificacion_de_dispensacion">Verificacion de dispensacion
                <option value="reproceso">reproceso
                <option value="Analisis_sensorial_cosmeticos">Analisis sensorial cosmeticos
...
if (isset($_POST['btn_consulta'])) {
    $serverName = "HERCULES"; //serverName\instanceName, portNumber (default is 1433)
   $connectionOptions = array(
    "Database" => "RBOSKY3",
    "Uid" => "SA",
    "PWD" => "Sky2022*!",
    "CharacterSet" => "UTF-8"  // Configura la codificación de caracteres
);

$serverName2 = "HERCULES";
...
$conn = sqlsrv_connect($serverName, $connectionOptions);
    ...
    $orden_input2 = $_POST['orden_input'];

    // Consulta basada en $orden_input
   $consultaOrden = sqlsrv_query($conn, "SELECT * FROM orden_de_produccion_matriz WHERE DocNum = '$orden_input' COLLATE Latin1_General_CI_AI");
    ...
    $consultaOrden2 = sqlsrv_query($conn2, "SELECT * FROM PROCESOS WHERE DocNum = '$orden_input2' COLLATE Latin1_General_CI_AI");
    $rowOrden2 = sqlsrv_fetch_array($consultaOrden2);

 $codigo = '';
    if (isset($rowOrden2) && !empty($rowOrden2['ItemCode'])) {
        $codigo = $rowOrden2['ItemCode'];
    } elseif (isset($rowOrden) && !empty($rowOrden['ItemCode'])) {
        $codigo = $rowOrden['ItemCode'];
    } elseif (isset($rowSAP) && !empty($rowSAP['ItemCode'])) {
        $codigo = $rowSAP['ItemCode'];
    }

    $Descripcion = '';
    if (isset($rowOrden2) && !empty($rowOrden2['ItemName'])) {
        $Descripcion = $rowOrden2['ItemName'];
    } elseif (isset($rowOrden) && !empty($rowOrden['ProdName'])) {
        $Descripcion = $rowOrden['ProdName'];
    } elseif (isset($rowSAP) && !empty($rowSAP['ItemName'])) {
        $Descripcion = $rowSAP['ItemName'];
    }



    if ($rowOrden || $rowSAP || $rowOrden2 ) {
...
?>
        ...
        <div class="nuevodiv">
            ...
            <input type="text" class="seleccion" name="ORDEN" value="<?php echo isset($rowOrden2) ? $rowOrden2['DocNum'] : $rowOrden['DocNum']; ?>" readonly>
            <label for="time" class="formulario__label">CODIGO SAP:</label>
            <input type="text" class="seleccion" name="CODIGO" value="<?php echo $codigo; ?>" readonly>
            <label for="time" class="formulario__label">DESCRIPCION DEL PRODUCTO:</label>
            <input type="text" class="seleccion" name="ARTICULO" value="<?php echo $Descripcion; ?>" readonly>
...
document.getElementById('myForm').addEventListener('submit', function() {
  ...
  document.getElementById('timeInput').value = horaActual;
			...
			<a href="http://192.168.2.242:8080/matriz_actualizada/php/archivo.php?"><input type="button" value="Consultar de unidades procesadas" id="enviar"></a>
			</div>
...
<body>
  <br>
    <div class="nuevo01">
        <center>
            <form method="post">
                ...
                <br>
                <center>
                <input style="    background-color: #a86a7b;
    ...
    border-radius: 7px;
    font-weight: 400;
font-size:18px;"  type="submit" value="Consulta" name="btn_consulta">
                </center>
            </form>
        </center>
    </div>
		...
		<script type="text/javascript">
			...
			var opt_Recepcion_muestreo_e_inspeccion = new Array ("-", "Ingreso, verificacion y registro de materiales","Muestreo e inspeccion de materiales","Muestreo e inspeccion de maquila","Muestreo de materias primas","Generacion de certificados","Limpieza y desinfección del área");
			var opt_Analisis = new Array ("-", "Analisis fisicoquimico de materia prima","Analisis fisicoquimico de producto","Analisis microbiologico de materia prima","Analisis microbiologico de producto","Analisis sensorial cosmeticos","Generacion de certificados","Limpieza y desinfeccion del area","Muestreos microbiologicos (aso)");
			var opt_Dispensacion = new Array ("-", "Despeje de linea y arranque de proceso","Dispensacion de materias primas","Dispensacion de materiales","Verificación de registros ");
			var opt_Fabricacion = new Array ("-", "Despeje de linea y arranque de proceso","Inyeccion de materiales","Preparacion de cosmeticos","Preparacion de dispositivo medico ( revelador)","Devanado","Encerdado de cepillos","Entorchado (ortodoncia)","Pulido de cepillos","Pulido de mangos","Verificacion de registros ","Verificacion orden, limpieza y desinfeccion","Ensamble limpiadientes","Marcacion de cepillos","Ensamble cepillos ortodoncia");
			var opt_Envasado_Ensamble = new Array ("-", "Despeje de linea y arranque de proceso","Envasado de cosmeticos","Envasado de dispositivo medico ( revelador)","Ensamble bobinas - carretas","Verificacion de registros","Ensamble sera brackets");
			var opt_Acondicionamiento_empaque = new Array ("-", "Despeje de linea y arranque de proceso","Codificacion (loteado)","Sellado dispositivos medicos","Armado de cajas, etiquetado e inserción cortador (sedas)","Etiquetado (calcas - bandas)","Troquelado","Armado de cajas y empaque de cremas y kits","Termoencogido","Empaque (bolsas,cajas, canastas)","Verificacion de registros ","Inspeccion y liberacion de producto terminado","Sellado kits (Blister)","Armado de kit (estuches)");
            ...
            var opt_Verificacion_orden_limpieza_y_desinfeccion = new Array ("Verificacion orden limpieza y desinfeccion");
            var opt_Verificacion_de_registros = new Array ("Verificacion de registros");
            var opt_Reunion = new Array ("Reunion");
            var opt_Capacitacion = new Array ("Capacitacion");
            var opt_Recibir_turno_calidad = new Array ("Recibir turno calidad");
            var opt_Entregar_turno_calidad = new Array ("Entregar turno calidad");
			var opt_Toma_de_Muestras_de_Materias_Primas = new Array ("Toma de Muestras de Materias Primas");
            var opt_Ingreso_y_verificacion_de_Material_de_empaque_y_Materia_Prima = new Array ("Ingreso y verificacion de Material de empaque y Materia Prima");
            var opt_Verificacion_de_dispensacion  = new Array ("Verificacion de dispensacion");
            var opt_reproceso  = new Array ("reproceso");
            var opt_Analisis_sensorial_cosmeticos  = new Array ("Analisis sensorial cosmeticos");
